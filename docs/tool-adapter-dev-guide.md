...
This guide explains how to add support for a new editor/IDE or tool to the `omniedge` CLI, so it can be configured via:

```bash
omniedge set <tool-name>
omniedge reset <tool-name>
```
...
Tool adapters live under `omniedge/tools/` and follow a common interface provided by `ToolIntegration` in `omniedge/tools/base.py`.

At a high level, a tool adapter:

- Knows where its configuration file lives (e.g. `~/.mytool/config.json`).
- Knows where to store backups (e.g. `~/.omniedge/backup/my-tool/`).
- Implements `set_config` to:
  - Create a backup of the current config (if any).
  - Update the config to use the OmniEdge API.
...
Create a new module under `omniedge/tools/`, for example:

```text
omniedge/tools/my_tool.py
```
...
class MyToolIntegration(ToolIntegration):
    ...
    def __init__(self) -> None:
        ...
        # Where OmniEdge should store backups for this tool
        self._backup_dir = Path.home() / ".omniedge" / "backup" / "my-tool"
...
- Updating it with OmniEdge‑specific values.
...
from omniedge.tools.base import ToolConfigSetResult
...
class MyToolIntegration(ToolIntegration):
    ...
    def set_config(self, *, base_url: str, api_key: str, model: str) -> ToolConfigSetResult:
        ...
        # 3) Apply OmniEdge‑specific configuration
...
To make the new tool available to users, register it in `omniedge/cli.py`.

1. Import your integration at the top of `omniedge/cli.py`:

   ```python
   from omniedge.tools.my_tool import MyToolIntegration
   ```
...
```bash
omniedge set my-tool
omniedge reset my-tool
```
...
1. **Setup flow**
   ...
   ```bash
   export OMNIEDGE_API_KEY="test-api-key"
   ...
   - Verify on disk:
     - `~/.omniedge/backup/my-tool/` contains a timestamped backup (if a config existed).
     - The tool's config file (e.g. `~/.mytool/config.json`) now includes the OmniEdge settings.
...
3. **Edge cases**
   ...
   - No backups: `omniedge reset my-tool` should print a clear message and exit without crashing.
...
- **Be conservative with user configs**
  - Only modify the fields needed for OmniEdge.
...
1. One new class under `omniedge/tools/`.
2. A couple of lines in `omniedge/cli.py` to register it.
